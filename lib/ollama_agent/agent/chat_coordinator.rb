  ...
  class Agent
    ...
    class ChatCoordinator
      ...
      def assistant_message(messages)
        if @agent.hooks.subscribed?(:on_token)
      ...
      def request_args(messages)
        base_chat_request_args(messages).tap do |args|
          th = ThinkParam.effective_for_model(@agent.send(:resolve_think), @agent.model)
      ...
      def base_chat_request_args(messages)
        {
          messages: messages,
          tools: OllamaAgent.tools_for(read_only: @agent.read_only, orchestrator: @agent.orchestrator),
          model: @agent.model,
      ...
      def block_assistant_message(messages)
        response = @agent.client.chat(**request_args(messages))
      ...
      def stream_assistant_message(messages)
        response = @agent.client.chat(**request_args(messages), hooks: ollama_stream_hooks)
        message = response.message
        raise EmptyAssistantMessageError, "Empty assistant message" if message.nil?
      ...
      def stream_on_thinking_hook(turn_proc)
        lambda do |fragment|
          @agent.hooks.emit(:on_thinking, { token: fragment.to_s, turn: turn_proc.call })
        end
      end
      ...
      def stream_on_token_hook(turn_proc)
        lambda do |*args|
          ...
          @agent.hooks.emit(:on_token, payload)
        end
      end

      def announce_assistant_content(message)
        @agent.hooks.emit(:on_assistant_message, { message: message })
        return if @agent.hooks.subscribed?(:on_assistant_message)
