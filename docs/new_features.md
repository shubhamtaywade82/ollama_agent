...
ruby
 ...
 2. **Supervisor Interceptor Layer:**
...
Keep a rolling compressed state of the conversation and task.

Inject only the minimum reconciliation delta from Claude:
...
Rebuild local context from disk snapshots, not from the old chat history.

Truncate by relevance, not by recency alone.
...
Prefer this order
Filesystem hash scan

Detect touched files

Load only changed files

Generate a compact reconciliation summary

Seed the local planner with that summary

Keep old Claude reasoning out of the prompt unless explicitly needed
...
DeepSeek V3 (API or high-VRAM nodes)
...
Using Prism is the correct choice for Ruby-first orchestration.
...
verify containment within allowed roots
...
external process swaps symlink

write occurs to forbidden target
...
**Objective:** Build the state machine that executes, verifies, and compensates mutations safely.
 ...
 2. **Isolated Verification (IsolatedValidator):**
...
**Objective:** Connect the probabilistic planner to the deterministic runtime safely.
 ...
 2. **Fast-Path Router Integration:**
...
resolve against ownership graph

validate single-context exclusivity

cache canonical ownership index
...
resolve nearest ownership root
